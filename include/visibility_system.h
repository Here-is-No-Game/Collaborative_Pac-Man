...
private:
    ...
    VisibleArea::CellContent getCellContent(const Position& pos, const GameMap& map, const std::vector<Character>& characters) const;

public:
    VisibilitySystem(int radius);

    // 计算指定位置的可见区域
    VisibleArea calculateVisibleArea(const Position& center, const GameMap& map, const std::vector<Character>& characters) const;
};
