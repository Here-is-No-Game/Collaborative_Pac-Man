...
inline std::wstring GbkToWide(const std::string &gbkstr) {
    ...
    int size_needed = MultiByteToWideChar(CP_ACP, 0, gbkstr.c_str(), (int)gbkstr.size(), NULL, 0);
    if (size_needed <= 0) return std::wstring();

    std::wstring wstrTo(size_needed, 0);
    MultiByteToWideChar(CP_ACP, 0, gbkstr.c_str(), (int)gbkstr.size(), &wstrTo[0], size_needed);
    return wstrTo;
}
